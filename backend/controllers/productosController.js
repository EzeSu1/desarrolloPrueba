...
class ProductosController {
    ...
    paginasTotales(req, res) {
        return ProductosService.paginasTotales()
            .then(cantidad_paginas => res.status(200).json(cantidad_paginas))
    }

    agregarStock(req, res) {
        const body = req.body
        const result_body = stockSchema.safeParse(body)

        if (!result_body.success) {
            return showBodyErrors(req, res, result_body)
        }

        return ProductosService.agregarStock(req.params.id, result_body.data.cantidad)
            .then(producto_actualizado => res.status(200).json(ProductosDTOs.productoToDTO(producto_actualizado)))
    }
}
